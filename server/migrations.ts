...
export async function migrateStripeFields() {
  try {
    ...
    // Check if stripe_customer_id column exists with retry logic
    const columnsResult = await withRetry(async () => {
    ...
    console.error('Error during Stripe fields migration:', error instanceof Error ? error.message : String(error));
